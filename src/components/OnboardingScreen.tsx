  ...
  const handleUnlock = async (silent = false) => {
    ...
    try {
      ...
      if (vaultUsesPrf) {
        if (!result.prfOutput) {
          throw new Error(
            "This vault requires PRF support. Your authenticator may not support it, or use a different key.",
          );
        }
          ...
          {!infoExpanded ? (
            ...
            <div className="text-left space-y-3 p-4 rounded-lg bg-card border border-border text-xs text-muted-foreground">
              ...
              <div className="space-y-2">
                <p>
                  <span className="text-foreground font-medium">What it does:</span> YubiMail creates disposable email
                  addresses using the{" "}
                  <a
                  ...
                  . Your login credentials for these addresses are encrypted and stored locally in your browser,
                  protected by your YubiKey.
                </p>

                <p>
                  <span className="text-foreground font-medium">Receive only:</span> These are receive-only inboxes. You
                  cannot send emails from them.
                </p>

                <p>
                  <span className="text-foreground font-medium">Email retention:</span> Emails on mail.tm servers are
                  stored for a maximum of 7 days, then automatically deleted. This is a limitation of the upstream
                  service.
                </p>

                <p>
                  <span className="text-foreground font-medium">What is encrypted:</span> Your mail.tm account
                  credentials (emails + passwords) are encrypted with AES-256-GCM inside the vault. The emails
                  themselves are not end-to-end encrypted - they are standard emails stored on mail.tm servers.
                </p>

                <p>
                  <span className="text-foreground font-medium">No backend:</span> YubiMail has no server. The encrypted
                  vault lives entirely in your browser's local storage. If you clear your browser data, your vault is
                  gone.
                </p>
              </div>
              ...
              <p className="pt-2 border-t border-border">
                ...
                <a
                  href="https://github.com/neonostr/yubimail"
                  target="_blank"
                  rel="noopener noreferrer"
